...
图形与文字平衡：文字是信息的主体。尽量少用图标和装饰性插画。每个技术点保留关键说明，但去掉冗余解释和重复表述，保持可读性。

信息结构：高密度纵向布局。模块之间用手绘线条和波浪箭头（hand-drawn wavy arrows）连接递进，表达流向关系。区块用不同背景色自然区分，不要画卡片框。重要：文字和视觉元素必须在画布上均匀分布，不能一侧密集一侧留白。如果某个区块内容少导致留白过多，用小的流程图、手绘指引线、标注气泡或装饰性元素填补，确保整体构图平衡。

文字层次：标题顶部居中，粗体大号手绘字（bold, large, hand-drawn lettering）；区域内粗体关键词 + 正常文字说明混排，要点化短句为主，关键处保留完整短语。
